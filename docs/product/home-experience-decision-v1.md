...
> **2026-09-02 방향 갱신** — Home 의 현재 방향(Home 1/2 역할, 전환 방식 재개봉 `OPEN_DECISION`)은 `home-discovery-external-ai-product-direction-ssot-v1.md`가 우선한다. 이 문서의 그 외 결정은 유지된다. 2026-09-02 UX 확정: 이 문서의 **1 route 좌우 스와이프 2면 구조**는 `home-ux-decision-ssot-v1.md`(Cover→Floor 수직 연속)가 supersede 한다.
