...
> **Owner 결정(2026-09-06) — 주간 freshness 범위 확대**: 기존 7일 주기 행사/이벤트 점검을
> **변동 가능한 Travel Essentials 까지 확대**한다. 매주 확인 대상 = 행사/축제/이벤트 ·
> 교통패스 · 공항/역 이동·리무진 · 관광안내소 · 짐보관 · 기타 여행 편의시설·서비스.
> 확인 항목 = 신규/변경/중단/취소/종료/가격/운영시간/이용조건/위치/공식 링크 변경.
> 문화유산·일반 관광지 같은 **안정적 기본 장소정보는 매주 전수 재검증 대상으로 확대하지 않는다.**
...
**My Trip ↔ Story 계약 (Owner correction, 2026-09-05)**

My Trip은 여행 기록과 편집의 본체다.

My Trip에서 수정한 사진·제목·메모·장소·일정 내용이
Story에 실시간으로 반영된다.

My Trip과 Story는 동일한 Trip의 동일한 이미지·텍스트 내용을 사용하며,
본질의 데이터가 다른 것이 아니라 **표현 방식**이 다르다.

- MY TRIP = 기록·편집의 본체
- STORY = 같은 My Trip 내용을 여행답고 세련된 방식으로 보여주는 표현

Story에서 제목·메모를 별도로 다시 생성하거나
별도 카피를 관리하는 구조가 아니다.

**My Trip 제목·메모의 AI 글 방향 — 3가지로 고정**

이 3가지는 "Story AI style"이 아니라,
My Trip에서 제목·메모를 작성/수정할 때 AI에게 요청할 수 있는 글 방향이다.
...
사용자가 원하는 방향을 선택한다.
AI가 제안한 제목·메모는 My Trip의 제목·메모다.
사용자가 직접 수정할 수 있다.
그 결과가 Story에도 동일한 내용으로 반영된다.
Story가 별도로 제목·메모를 생성하는 기능이 아니다.
...
→ 네이버/Google 지도 길찾기
→ 사진 촬영 또는 갤러리에서 추가
→ My Trip 제목·메모의 AI 글 방향 3가지 선택
→ My Trip에서 만든 이미지/텍스트가 Story에 반영됨
