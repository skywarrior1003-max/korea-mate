...
3. **STAGE** (`--stage`, service_role REST chunk): MATCH 462 → 기존 id PATCH(source-owned 필드만, is_published 미전송) · NEW 4,194 → `is_published=false` upsert `on_conflict=source_type,external_id` · legacy 233 은 그대로 true. 산출: `production-runs/<run_id>/production-id-mapping-v1.jsonl`(canonical_id → actual id) + `stage-receipt-v1.json`. 검증: post = pre + 4,194 · mapping 4,194 · 전부 false · 삭제 0.
...
| NEW identity | `identity.ts`: `source_type="canonical"`, `external_id="<city>:<canonical_id>"` — 012 `idx_city_spots_source_external` UNIQUE 가 DB-level 멱등 보호. package 버전 비결합 | `PACKAGE_VERSION_COUPLED_IDENTITY=NO` |
