...
**migration 수동 적용 운영 계약 · DECIDED** 이 프로젝트는 005 이후를 `supabase db push` 가 아니라 검증된 SQL 수동 적용으로 운영한다. 그래서 로컬 파일 수와 원격 history 기록 수가 다른 것이 **정상**이다. 이 차이는 **결함이 아니라 운영 모델의 결과**이며, 그 자체로 어떤 작업의 차단 사유도 아니다.

실측(`supabase migration list --linked`, CLI 2.111.0, project `tfulaxxtorbxhlgupktc`)

| 시점 | 로컬 파일 | 원격 history 기록 | 공백 구간 |
|---|---|---|---|
| 2026-07-28 | 31개 (001~031) | 4건 (001~004) | 005~031 |
| **2026-08-05 (현재, `cf48274`)** | **34개 (001~034)** | **4건 (001~004)** | **005~034 (30건)** |

- **금지** `supabase db push` · `supabase migration repair` · `supabase_migrations.schema_migrations` 직접 INSERT·UPDATE·DELETE · history 기반 자동 적용
- **금지** 원격 history 공백만을 이유로 한 자동 repair. 공백은 예상된 상태다
- **허용** 사전 스키마 직접 조회 → **사용자 명시적 승인** → 검증된 SQL 수동 적용
- **적용 기록 필수 항목** migration 파일명 · 적용 대상 project · **사전 schema fingerprint** · 적용 명령 · 적용 시각 · 적용 결과 · **사후 schema fingerprint** · rollback SQL · QA 결과
- **적용 전후 필수** baseline assertion · 사후 assertion · rollback SQL · 운영 회귀 QA
- migration 파일은 **코드·감사·재현용 이력**으로 보존하고, 실제 적용 여부는 **DB 구조·assertion·role 권한·fingerprint** 로 검증한다

> **HARD STOP** 이 저장소에서 `supabase db push`, `supabase migration repair`, migration history 직접 조작을 실행하지 않는다. 작업 프롬프트가 있더라도 사용자에게 **해당 명령의 실행을 명시적으로 승인받지 않은 한** 금지한다.

**034 사례 (2026-08-05)** `034_secure_events_rls.sql` 은 위 계약대로 사용자 승인 → 드라이런(ROLLBACK) → 수동 적용 → 사후 assertion·fingerprint·회귀 QA 까지 완료했다. **원격 history 에 기록하지 않은 것이 계약과 일치하는 정상 상태다.** 034 만 applied 로 표시해도 005~033 29건은 그대로 남아 위험이 줄지 않고, 이력만 불연속이 된다.

**자동 migration 체계로 전환하려면** 005 이후 전체 migration 을 DB 구조로 전수 대조한 뒤 **별도 사용자 승인**을 받아야 한다. 부분 repair 로 접근하지 않는다.
