...
| 15 | Schedule Confidence Gate | "Generated" ≠ "good enough to show" | Day/trip → internal score → recompute / substitute / unplaced / ask | `PARTIAL` (D-16) — P1 minimal: `computeScheduleConfidence` per Day (coordinate · hard-constraint · route · backtracking → GOOD/REVIEW/FAIL, `ScheduledDay.quality`), internal only (`[plan-timing]` log; never rendered). Threshold actions limited to the single repair pass; no substitute/unplaced/ask flow | P1 (minimal) |
...
| Coordinate confidence | `PARTIAL` | P0 gate `isSchedulableCoordinate` (Korea bounds) for candidates + This Trip hints; P1 v3 re-verify 55 targets → 5 EXACT correction candidates (Owner SQL `OWNER-RUN-P1-busan-corrections-v2.sql` pending), 33 STREET_ONLY + 11 NOT_FOUND + Igidae 7 HOLD; Naver Local Search 401 (not bypassed); no confidence fields in `city_spots` | P1 |
| Neighborhood clustering | `PARTIAL` | P1 coordinate leader clusters (1 km / adjacent 2 km, city-agnostic) → cluster-focused Day supply (limit 60) + in-Day affinity/re-entry/focus weights + one STRICT repair; distance zones kept underneath; no named-neighborhood model (by design) | P1 |
| Backtracking audit | `PARTIAL` | P1 product-side P3.6 `auditDayRoute` (JUSTIFIED/UNJUSTIFIED, cluster re-entry) + gate with ≤1 repair; 10-day Busan (P1 detector, isolated dataset) unjustified 19→8, re-entries 10→4 | P1 |
| Route quality audit | `PARTIAL` | P1 gate in product (`runScheduler`); 10-day Busan 51.2→44.4 km, 812→763 min, REVIEW days 5→4; residual unjustified re-entries Days 6/7/9/10 → `SCHEDULER_ROUTE_QUALITY_PASS=NO` (D-26) | P1 |
| Schedule confidence | `PARTIAL` | P1 minimal `computeScheduleConfidence` GOOD/REVIEW/FAIL per Day, internal/log only; no substitute/unplaced/ask flow | P1 |
