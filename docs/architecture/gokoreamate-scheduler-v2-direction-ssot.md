...
**(D-33, Production write APPLIED 2026-08-31 06:42 UTC, Owner-authorized)** `OWNER-RUN-P1-busan-corrections-v3.sql` executed as one transaction (precheck 6/6 → 5 coordinate UPDATEs → fee UPDATE → postcondition 6/6): READ-BACK 6/6 TARGET_MATCH — 1127/136 → (35.2217587, 129.0835634), 340 → (35.1569143, 129.1460571), 386 → (35.1616748, 129.1673516), 1167 → (35.1598007, 129.169609), id 2 entry_fee `₩2,000` → `Free`; name/address/source_type/is_published/category unchanged; rows touched in the last 30 min = 6, Busan published rows 807 unchanged. POST-WRITE 10-day (real rows, harness): 28,667 m / 685 min / unjustified 0 / re-entry 0 / zigzag 0 / GOOD 5 · REVIEW 5 (all `COORDINATE_UNCERTAINTY` only: 1135 Day 2 · 334 Day 6 · 1133+272 Day 7 · 395+397 Day 8 · 1129 Day 9) / FAIL 0 / ≥20 % better order 0 / protected loss 0 / fixed violations 0; corrected rows 1167 (Day 6) and 340 (Day 8) now schedule on land. Live app 10-day 3.4–3.8 s; live loop (chains days from the previous day's end — Stay is `PARTIAL`, no morning reset) 9 GOOD · 1 REVIEW (Day 9 Yeongdo: 절영해안산책로 → 영도대교 → 봉산마을 reversal 1.6 km/0.85 km, `UNJUSTIFIED_ROUTE`, better-order ratio 0 because the travel-time table quantizes both orders equally — route-cost quantization, not data; not fixed in this task). Gamcheon: Explore card shows `Free` live; `/place/2` is a prerendered static page — the rebuilt export renders `Free`, Production shows `Free` only after the next deploy. Determinism finding: Day 6 (Production rows) primary-cluster mass tie (c2 = c5 = 1022.000) → 8/20 shuffles pick a different (equally GOOD) day — needs a deterministic tie-break in `scheduleOnce` (follow-up; not in this task). Sea rows 1135, 1133, 272, 395, 397 still scheduled → `KNOWN_SEA_MARKER_RELEASE_BLOCKER=YES`.
**(D-34, release closure 2026-08-31)** (a) **Stay lifecycle is not a bug**: contract = `stayStartFor` — a day starts at the stay that covers the previous night (`checkIn < D ≤ checkOut`), otherwise it carries forward from the previous day's last stop; `stays[]` is built from the `stayArea` preset value (e.g. `Haeundae (해운대)`) plus the TripDraft exact coordinate. The earlier "live ≠ harness" observation came from QA URLs without a valid preset / arrival coordinates. With the canonical URL the live client sends Day 1 = arrival (35.1148, 129.042), Days 2–10 = stay (35.1589, 129.16), last day = departure fixed event 19:57 — identical to the harness (`HARNESS_LIVE_SEMANTICS_MATCH=YES`). (b) **Determinism**: primary-cluster mass tie now breaks by the smaller cluster id (`engine.ts`), 20/20 shuffles identical on every Production day. (c) **Detector**: a cluster switch/re-entry counts only across a leg ≥ MIN_LEG_M (800 m) — a 50 m hop over a cluster boundary is not "leaving" (removed the Gwangalli false positive). (d) **Sea rows 272, 395, 397, 400, 1133, 1135, 1138 → EXACT_VERIFIED** at road-address house-number level via the VWorld (government) geocoder, cross-checked with the P1 OSM street geocode (29–172 m for 6 rows; 1135 resolved to house number 193 where OSM had the road only) — artifact `owner-approved-corrections-v3/owner-approved-corrections-v4-busan-sea7.json`, self-verifying SQL `OWNER-RUN-P2-busan-sea7-corrections-v4.sql` (7 rows, rollback values inside) — **not executed** (PROD-SQL task). (e) Real Production rows + stay preset, corrected detector: unjustified 0 / re-entry 0 / zigzag 0 / GOOD 6 · REVIEW 4 (coordinate uncertainty only) / live API 10/10 GOOD / 3.3 s; with v4 applied as test input: GOOD 8 · REVIEW 2 (inland STREET_ONLY rows 334, 1129) / 0 route defects. Remaining release blocker = the 7 sea rows until the v4 SQL is applied (Day 2 map still shows 달타이 in the sea) and the Owner's visual acceptance.

**Current Checkpoint (2026-08-31, final product QA — D-35).** Sea-7 v4 SQL applied in Production (READ-BACK 7/7, 07:31 UTC); known problematic set (12 rows) all on land — `KNOWN_SEA_MARKER=0` for the known set (not a Busan-wide audit). Real Production rows, canonical stay preset, no isolated dataset: harness unjustified 0 / re-entry 0 / zigzag 0 / GOOD 8 · REVIEW 2 (`COORDINATE_UNCERTAINTY` only: inland STREET_ONLY rows 334, 1129 — DATA/COORDINATE, non-blocking) / FAIL 0; live API 10/10 GOOD; live 10-day ×5 and 3-day ×5 identical; KO core flows (Explore → Save → Picks → This Trip → Setup → Generate → Planning → Day map → fullscreen → marker/modal → Edit → Today → save/reopen → Story), EN/JA/ZH, 5 cities, Gamcheon Free (card + `/place/2` in the feature build) all pass, pageerror 0, SDK 401 0. **CLEARED**: Scheduler · Stay · determinism · known sea coordinates · semantic QA. **HOLD**: Owner visual acceptance · feature → master / Production release (the Production static `/place/2` shows the pre-correction fee until the next deploy). **Owner data decision (non-blocking)**: duplicate published rows seen in one day — 1348/1438 (분홍이네, same address) and 1362/1335 (요트탈래 / 부산요트투어(요트탈래), same building); identity merging is not done by code (Final data authority) — dedupe/unpublish is a data decision, or a same-day near-duplicate scheduler rule if the Owner prefers. P2 direction unchanged (Naver real travel-time, opening-hours feasibility, Intent Compiler, Constraint Negotiator).
**Current Checkpoint (2026-08-31, Owner handoff — D-37).** Duplicate correction v5 applied in Production (1438/1362 unpublished, 1348 shows `분홍이네`, 1335 at The Bay 101). Real Production data, canonical stay preset, no isolated dataset: harness unjustified 0 / re-entry 0 / zigzag 0 / GOOD 8 · REVIEW 2 (inland coordinate uncertainty only) / same-business duplicate in a day 0; live app 10-day ×5 identical, 3-day ×5 identical, live API 9 GOOD · 1 REVIEW (Day 9: 누리마루 → 3355마린 → 광안리, one 1.2 km reversal that the scheduler's travel-time table scores as cost-optimal — real but minor, classified REAL ROUTE DEFECT/minor, not repaired by the gate). Browser QA KO core flows + EN/JA/ZH + 5 cities + Gamcheon Free all pass. **CLEARED**: Scheduler · Stay · determinism · known coordinates · targeted duplicates · final semantic QA. **NEXT**: Owner visual approval (local canonical URL `scratchpad/canonical-url.txt`; a feature preview deploy is an Owner action) → separate release cutover. Production `/place/*` static pages refresh on the next feature deploy.
...
| Stay | `CURRENT_IMPLEMENTATION` (day start) / `PARTIAL` (scoring) | `stayStartFor`: each day starts at the stay covering the previous night, else carry-forward (verified live = harness, D-34); check-in second pass Day 1 only with confirmed coordinate (`planCheckin`); no stay-relative route scoring | P1 |
...
| Coordinate confidence | `PARTIAL` | P0 gate `isSchedulableCoordinate` (Korea bounds) for candidates + This Trip hints; **5 EXACT rows + Gamcheon fee corrected in Production 2026-08-31 (D-33, READ-BACK 6/6)**; 33 STREET_ONLY + 11 NOT_FOUND + Igidae 7 HOLD; 7 published sea rows (272, 395, 397, 400, 1133, 1135, 1138) = My Trip RELEASE_BLOCKING (D-32/D-33); no confidence fields in `city_spots` | P1 |
...
| Route quality audit | `CURRENT_IMPLEMENTATION` | closure V2 (D-31) + release closure (D-34): real Production rows with the stay preset → unjustified 0 / re-entry 0 / zigzag 0, GOOD 6 · REVIEW 4 (coordinate uncertainty only), live API 10/10 GOOD; primary-cluster tie-break + ≥800 m cluster-switch rule in place; 7 sea rows correction-ready (v4 SQL, PROD-SQL task) | P1 |
