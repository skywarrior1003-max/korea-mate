...
**예산값의 지위 (CORRECTION-V1 §4)**: 072 초기값 `$5/day`·`$60/month` 는 모든
스위치가 off 인 상태의 **기술적 초기값**일 뿐이다.

- 사용자 이용권·판매량 정책이 아니고, 최종 사업 예산으로 승인된 값도 아니다.
- live 전환 전에 결제 사용자 수·무료 사용자 수·route 별 실제 최악 비용으로
  **다시 산정해 Owner 가 승인**해야 한다.
- 스위치를 'live' 로 켜는 것과 예산을 승인하는 것은 **별개의 행위**다 — 스위치를
  켜기 전에 이 두 값이 승인된 숫자인지 먼저 확인하라.
- 최종 숫자 확정은 AI live 전환 TASK 의 Owner 확인 항목이다.
