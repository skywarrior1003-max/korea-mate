...
test("V3: 끝난 여행의 일정은 읽기 전용 — 편집·추가·완주 토스트가 숨는다", () => {
  ...
  // TIMELINE-B-R1 이후 메뉴 항목은 편집 중이면 "편집 마치기"(종료)까지 겸해 조금 길어졌다 — 조건은 그대로다.
  assert.match(page, /\(!shareId \|\| isOwner\) && !isPastTrip\s*\?\s*\[\{ key: "edit"[\s\S]{0,600}setViewMode\("compact"\)/);
