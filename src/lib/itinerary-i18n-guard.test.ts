...
test("★migration 집합 스냅숏 — 승인된 75개 그대로, 예상 밖 추가/삭제 없음", () => {
  ...
  //  · 072     — Staging+Production 적용 CLOSED(CORRECTION-V1 PHASE B)
  //  · 073·074 — PLACE-USAGE-FOUNDATION-V2, Staging-only(Production 은 별도 릴리스)
  const dir = join(ROOT, "supabase", "migrations");
  const files = readdirSync(dir).filter(f => f.endsWith(".sql")).sort();
  assert.equal(files.length, 75, `migration 수가 변했다: ${files.length}`);
  assert.ok(files.includes("041_lock_down_legacy_spots_select.sql"));
  assert.equal(files[files.length - 1], "075_place_usage_monthly_refresh_exact.sql",
    "075(refresh exact) 가 마지막이어야 한다");
  // 번호 공백·중복 금지: 001..072 가 정확히 한 번씩.
  const nums = files.map(f => f.slice(0, 3));
  assert.equal(new Set(nums).size, 75, "번호 중복");
  for (const f of files.filter(f => f.slice(0, 3) > "041")) {
    assert.match(f, new RegExp(
      ...
      "072_ai_ops_ledger_and_switches|073_place_usage_annual_key|074_place_usage_monthly|075_place_usage_monthly_refresh_exact)\\.sql$"),
      `예상치 못한 migration: ${f}`);
  }
});
