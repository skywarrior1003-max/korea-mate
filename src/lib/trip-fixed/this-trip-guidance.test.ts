...
test("K·L This Trip 제거는 cart 만 건드린다", () => {
  ...
  // PICKS-TO-TRIP-JOURNEY-RESTORE-V1: 목록과 같은 도시 기준(viewCity)으로 뺀다 — 의미는 그대로다.
  assert.match(picks, /aria-label=\{`\$\{t\("removeFromTrip"\)\}: \$\{item\.name\}`\}[\s\S]{0,90}removePlaceFromThisTrip\(item, viewCity\)/);
