...
//   그래서 Functions 가 쓰는 순서를 attribute 없는 스냅숏(editorial-spot-order.json)
//   으로 분리했다. 원천과의 동기화는 ranking-ux-guard.test.ts 의 sync 테스트가
//   지킨다 — regional-places 를 고치면 스냅숏도 함께 재생성해야 테스트가 통과한다.
//
...
import editorialOrderRaw from "../../data/regional/editorial-spot-order.json";

const ORDER = editorialOrderRaw as Record<string, number[]>;
