...
// 출력  ImportPlan — UPDATE(기존 id 보존) · INSERT(NEW) · SKIP(AMBIGUOUS/EXCLUDED) · NO_WRITE(legacy)
//
// 지키는 것
...
import { isMainCategory } from "./category-adapter.ts";
...
export interface CrosswalkRow {
  ...
  decision: "MATCH_REPLACE" | "NEW" | "AMBIGUOUS" | "EXCLUDED";
  tier: string;
  twin_of?: string | null;
}
...
export interface ImportPlan {
  ...
  per_city: Record<string, { before: number; updates: number; inserts: number; projected_after: number }>;
...
export function planImport(input: {
  ...
  const policyCounts: Record<NullPolicy, number> = { REPLACE_WITH_VALUE: 0, INTENTIONALLY_CLEAR: 0, NO_SOURCE_VALUE: 0, PRESERVE_RUNTIME_FIELD: 0, MANUAL_REVIEW: 0 };
  ...
  for (const c of sortBy(input.crosswalk, x => `${x.city}|${x.canonical_id}`)) {
    ...
    if (c.decision === "AMBIGUOUS") {
      skips.push({ action: "SKIP", canonical_id: c.canonical_id, city: c.city, reason: `AMBIGUOUS:${c.tier}`, twin_of: c.twin_of ?? null });
      continue;
    }
  ...
  for (const m of sortBy(input.main, r => String(r.main_city_spot_id).padStart(8, "0"))) {
    if (!targeted.has(m.main_city_spot_id)) noWrite.push({ action: "NO_WRITE", main_city_spot_id: m.main_city_spot_id, city: m.city, reason: "legacy/duplicate/excluded — untouched, not deleted" });
  }
  ...
  for (const city of [...cities].sort()) {
    ...
    per_city[city] = { before, updates: u, inserts: i, projected_after: before + i };
  }
  return {
    updates, inserts, skips, no_write: noWrite,
    counts: {
      active_input: active.length,
      match_replace: updates.length,
      new: inserts.length,
      ambiguous_skipped: skips.filter(s => s.reason.startsWith("AMBIGUOUS")).length,
