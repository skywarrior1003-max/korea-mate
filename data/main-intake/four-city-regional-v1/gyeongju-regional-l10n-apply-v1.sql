...
-- 대상(v3): 경주 regional 14행 — EN 제목 14 + **EN 본문 14**(상류 수집 원본
--        gyeongju-15-en-multilingual-v1.jsonl, KTO EngService2/visitkorea, CONFIRMED)
--        + 432/530 JA·ZH(공식 native: gyeongju.museum.go.kr · UNESCO).
-- 근본 원인: 최종 패키지 조립 시 EN description 이 누락·오결합(0009/0127 zh 혼입)
--        — 상류 원본에는 정상 EN 본문이 15행 중 14행 존재(0099 만 SNA).
-- v2(박물관/UNESCO EN 본문 2행)는 수집 원본 계보로 통일하며 대체(SUPERSEDED).
-- GJ01-0099(511)는 EN 원천 부재 — 대상 밖(창작 금지).
...
UPDATE city_spots SET desc_l10n = coalesce(desc_l10n,'{}'::jsonb) || jsonb_build_object('en','Gyeongju National Museum houses numerous historical and cultural artifacts of the Silla dynasty (57 BC-AD 935). The museum provides various programs including at the Children’s Museum School. The newly renovated Silla Art Gallery and Silla History Gallery lobby by Teoyang Studio are popular among visitors. This multi-complex center provides the history of Silla with various artifacts.'), updated_at = now()
...
UPDATE city_spots SET desc_l10n = coalesce(desc_l10n,'{}'::jsonb) || jsonb_build_object('en','To the southeast of the royal tomb, is a small monument that has been erected among the pine trees; next to the monument is a well called Najeong. According to Samguksagi (Historical records of the Three Kingdoms) and Samgungnyusa (Memorabilia from the three dynasties), Park Hyeokgeose, the founding monarch of Silla, was born by this well. In 69 BC, Sobeolgong, the head of Goheochon Village, saw a white horse on its knees by the well. When he approached the well he found that the horse had magically disappeared and that a large egg was left in its place, from which a boy was born. When the boy turned 13 years old (57 BC), he was appointed king by the village chiefs and began to rule the area then called ‘Seorabeol’. A memorial stone (2.25 meters high, 45 centimeters long, and 21 centimeters wide) was erected in 1803 in the third year of King Sunjo''s rule (Joseon dynasty) detailing the historical origins of the founding father of Silla.'), updated_at = now()
 WHERE id = 468 AND city = 'gyeongju' AND NOT coalesce(desc_l10n,'{}'::jsonb) ? 'en';
...
UPDATE city_spots SET desc_l10n = coalesce(desc_l10n,'{}'::jsonb) || jsonb_build_object('en','The Five Royal Tombs (called Oreung in Korean) have been officially designated Historic Site No. 172 and are the final resting places of four kings of the Park clan—King Park Hyeokgeose (founder of the Silla Kingdom), King Namhae, King Yuri, and King Jabi—and one queen (Queen Aryeong, wife of King Park Hyeokgeose). 

To the east of the royal tombs lies Sungdeokjeon Shrine, which holds the ancestral tablet of King Park Hyeokgeose. Behind the shrine is the Aryeongjeong Well, said to be the birthplace of Queen Aryeong.'), updated_at = now()
 WHERE id = 475 AND city = 'gyeongju' AND NOT coalesce(desc_l10n,'{}'::jsonb) ? 'en';
...
UPDATE city_spots SET desc_l10n = coalesce(desc_l10n,'{}'::jsonb) || jsonb_build_object('en','Samneung meaning "three royal tombs," has strong ties to history. The three royal tombs house three kings of the Silla Kingdom: King Adala (8th King of the dynasty), King Sindeok (53rd) and King Gyeongmyeong (54th).   King Adala, who cared deeply for his people, went to war after his citizens were abducted by the invaders of Baekje. But when Baekje asked for a peace treaty, King Adala released the prisoners he took during the engagement. During his rulling, a  kingdom in Japan sent an envoy to ask for friendly relations with Silla. King Adala’s tomb is 58m in circumference at the base, 5.4m in height and 18m in diameter.   When King Hyogong died without any heirs, the people of the kingdom crowned his son-in-law as their next king – King Sindeok. During his reign, King Sindeok devoted himself to protecting his kingdom from invasions by Gyeonhwon and Gungye. The royal tomb is 61m in circumference at the base, 5.8m in height and 18m in diameter. It was robbed twice, inviting investigations in 1953 and 1963. The investigations revealed the tomb to be a chamber made of stone. King Gyeongmyeong, the son of King Sindeok, together with Wanggeon, the founder of the Goryeo dynasty, successfully defeated Gyeonhwon’s attack against Daeyaseong Fortress. During his reign, he attempted to establish diplomatic relations with the Hudang dynasty of China, but was unsuccessful. The tomb is 50m in circumference at the base, 4.5m in height and 16m in diameter.'), updated_at = now()
 WHERE id = 665 AND city = 'gyeongju' AND NOT coalesce(desc_l10n,'{}'::jsonb) ? 'en';
...
BEGIN
  ...
  SELECT count(*) INTO en_desc_cnt FROM city_spots
   WHERE id IN (425,432,436,439,444,454,457,462,468,473,475,481,530,665) AND coalesce(desc_l10n,'{}'::jsonb) ? 'en';
  IF en_cnt <> 14 OR ja_cnt <> 2 OR zh_cnt <> 2 OR en_desc_cnt <> 14 THEN
