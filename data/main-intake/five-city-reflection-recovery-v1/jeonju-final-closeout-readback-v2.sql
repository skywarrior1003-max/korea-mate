-- jeonju-final-closeout-readback-v2.sql (READ-ONLY, 적용 직후)
-- 기대: v1 항목(total 236·published 211·excluded_unpub 25·rows_kept 25·rel 224·eligible 224·prim 211·
--   spot_img 211·published_without_img 0·dup 0·multi_prim 0·https 224) + kto_official 174 ·
--   unknown_left 0 · unknown_eligible 0 · note_history_kept 174
select
 ...
 (select count(*) from public.city_spot_images i join public.city_spots c on c.id=i.city_spot_id where lower(c.city)='jeonju' and i.rights_status='KTO_OFFICIAL') as kto_official,
 (select count(*) from public.city_spot_images i join public.city_spots c on c.id=i.city_spot_id where lower(c.city)='jeonju' and i.rights_status in ('RIGHTS_UNKNOWN','KTO_TYPE_UNKNOWN')) as unknown_left,
 (select count(*) from public.city_spot_images i where i.display_eligible and i.rights_status in ('RIGHTS_UNKNOWN','KTO_TYPE_UNKNOWN')) as unknown_eligible,
 (select count(*) from public.city_spot_images i join public.city_spots c on c.id=i.city_spot_id where lower(c.city)='jeonju' and i.rights_status='KTO_OFFICIAL' and i.rights_note like '%cpyrhtDivCd 미확인%') as note_history_kept;
